...
// Функция, которая возвращает строку с командой движения влево или вправо
const getDirection = (distance, direction) => direction === 'L' ? `←${dv}` : `→${dv}`;

// Функция для вычисления маршрута до цели
const getRoute = (arr) => {
    const [x, y] = arr;

    // Вычисление средней позиции между x и y, разделенной на скорость
    const average = (x + y) / (2 * speed);

    // Выбор направления в зависимости от минимального значения координаты
    const min = Math.min(x, y);
    const direction = Math.sign(min) === -1 ? 'L' : 'R';

    return getDirection(Math.abs(average), direction);
}

function AI(game) {
    while (game.inspectTG() !== "All Cleared!") {
        // Получаем информацию о текущем состоянии игры
        const { lives, currentLevel, objects } = game.inspectTG();

        // Находим цель (goal) среди видимых объектов
        const goal = objects.find((i) => i.type === 'goal');
        const coords = goal.location;

        // Вычисляем маршрут до цели и команду для попытки достижения цели
        const route = getRoute(coords);
        const goalReachAttempt = 'X1';

        // Выполняем команду движения и попытку достижения цели
        game.press(route);
        game.press(goalReachAttempt);
    }
}

const CWRunnerExample = new TooGameCWR([
    [{ type: 'goal', location: [100, 110], height: 0 }],
    [{ type: 'goal', location: [-110, -100], height: 0 }]
]);

AI(CWRunnerExample);

const final_status = CWRunnerExample.getFullInfo(); // ###### Not allowed in real test cases.

describe('Alive', function () {
    console.log(`Lives: ${final_status.lives}`);
    Test.assertDeepEquals(!!final_status.lives, true, 'You DIED!!!');
});

describe(`${CWRunnerExample.basic_levels} Example Test Cases`, function () {
    for (let i = 0; i < CWRunnerExample.basic_levels; i++)
        it(`Level ${i + 1}`, function () {
            console.log(final_status.levels[i]);
            Test.assertDeepEquals(final_status.level_status[i], true, 'Failed!');
        });
});
...
В игре CodeWarsRunner есть несколько уровней, каждый из которых имеет цель (goal), которую нужно достичь.
Персонаж двигается горизонтально со скоростью 10 единиц в секунду.
Уровни могут быть разными, и игрок должен двигаться влево или вправо, чтобы достичь цели на каждом уровне.

Для получения информации о текущем состоянии игры используется метод game.inspectTG(), 
который возвращает объект с данными о жизнях, текущем уровне и видимых объектах на уровне.
Видимые объекты представлены в формате массива объектов, где каждый объект имеет свойства типа (type), координат (location) и высоты (height).
Координаты цели (goal) считываются из объектов, где type равен 'goal', и используются для определения маршрута до цели.
Скорость движения персонажа (10 единиц в секунду) задана как глобальная константа speed.
Общий подход в коде заключается в том, что он строит маршрут до цели на каждом уровне, учитывая текущие условия уровня, и затем выполняет необходимые команды для движения и достижения цели на каждом этапе игры.

Решение:

Создается функция getDirection(distance, direction), которая принимает два аргумента: 
- distance (расстояние для движения) 
- direction (направление: 'L' для влево или 'R' для вправо). 

Функция возвращает строку с командой движения в заданном направлении с указанным расстоянием.
Создается функция getRoute(arr), которая принимает массив arr с координатами цели (goal). 
Функция вычисляет маршрут до цели, определяя направление движения (влево или вправо) и расстояние до цели. 
Затем она использует getDirection для получения команды движения.

Основная функция AI(game) управляет процессом игры:
В бесконечном цикле (while) проверяется текущее состояние игры с помощью game.inspectTG(), 
и игра продолжается до тех пор, пока не достигнут статус "All Cleared!" (все уровни пройдены).
Внутри цикла получается информация о текущем состоянии игры, 
включая количество жизней (lives), текущий уровень (currentLevel) и видимые объекты (objects).

Из видимых объектов находится цель (goal) с типом 'goal', и ее координаты сохраняются в переменной coords.
Далее вызывается функция getRoute(coords) для вычисления маршрута до цели, и результат сохраняется в переменной route.
Выполняются две команды с помощью game.press(): первая - для движения по рассчитанному маршруту (route), вторая - для попытки достижения цели ('X1').
